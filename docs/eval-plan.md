...
| --- | --- | --- | --- | --- | --- |
| Paperclip | `claude-agent-acp` with managed `CLAUDE_CONFIG_DIR` seeded from `~/.claude/settings.json`+`CLAUDE.md`; `settingSources: user, project, local` | one per heartbeat | project layer (`boxer install claude-code`) or `--user`; Codex via `--user` inline hooks | `internal/eval/orch_paperclip.go`: `paperclipai test-drive --harness claude --no-browser --data-dir <work>` then, over REST, `adapterConfig.env`, `enableIsolatedWorkspaces`, project, issue, and a wake naming the issue | yes, T1 and T2 (live 2026-09-18: pass, 2 m 4 s, $0.0364). `BOXER_TRACE` does not survive `adapterConfig.env`, so the oracle judges by VM + guest canary |
| T3 Code | provider CLIs as ACP subprocesses; `CLAUDE_CONFIG_DIR` set to a T3-owned dir (`ClaudeHome.ts`) | per thread ("New worktree") | project layer, or the instance's `binaryPath` pointed at a `boxer shim` (inside) | `internal/eval/orch_t3.go`: `t3 serve --base-dir <work>`, a bearer session from `t3 auth session issue`, then Effect RPC over the WebSocket — `project.create`, `thread.turn.start` with `bootstrap.createThread` + `bootstrap.prepareWorktree`, `subscribeThread` | yes, T1 and T2, two cells (live 2026-09-18: rewrite pass 18 s $0.0047, inside pass 27 s $0.0070). The turn ends when `thread.session-set` clears `activeTurnId`, not at the final message |
...
| Paperclip: a wake must carry `payload.issueId` or the run resolves no project and uses a fallback workspace on the host; `enableIsolatedWorkspaces` (experimental instance setting, off by default) gates the project's per-issue worktree | `eval/orch_paperclip.go`, `docs/orchestrators.md` |
| T3: the assistant's final `thread.message-sent` is empty; the turn ends when `thread.session-set` clears `activeTurnId` | `eval/orch_t3.go`, `docs/orchestrators.md` |
| An orchestrator's worktree is known only at run time, so the driver records it in `Env.Root` and the oracle resolves scope, config and VM from there | `eval/eval.go`, `timing.go`, `oracle.go` |
